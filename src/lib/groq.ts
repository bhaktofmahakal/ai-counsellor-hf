...
export const FALLBACK_MODEL = process.env.GROQ_FALLBACK_MODEL || 'qwen/qwen3.8-27b';
...
export async function streamOnboardingResponse(
  ...
  try {
    ...
    if (error?.status === 429 || error?.status === 404 || error?.code === 'model_not_found') {
      console.log('Falling back in onboarding stream to:', FALLBACK_MODEL);
...
export async function generateAIResponse(
  ...
  try {
    ...
    // Automatic Fallback on Rate Limit or Model Not Found
    if (error?.status === 429 || error?.status === 404 || error?.code === 'model_not_found') {
      try {
...
export async function streamAIResponse(
  ...
  try {
    ...
    if (error?.status === 429 || error?.status === 404 || error?.code === 'model_not_found') {
      console.log('Falling back to fallback model due to error...', error?.status, 'Fallback:', FALLBACK_MODEL);
      try {
        return await groq.chat.completions.create({
          messages: [
            { role: 'system', content: systemPrompt },
